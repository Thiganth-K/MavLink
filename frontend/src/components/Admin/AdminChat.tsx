...
type Msg = { _id: string; from: any; to: any; content: string; createdAt: string; read?: boolean };
...
export default function AdminChatModal({ onClose }: Props) {
  ...
  useEffect(() => {
    const doLoad = async () => {
      try {
        ...
        // mark unread messages addressed to this admin as read
        const toMark = (msgs || []).filter(m => m.to && m.to.role === 'ADMIN' && !m.read).map(m => m._id);
        if (toMark.length) {
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      await chatAPI.adminSend(trimmed);
      setInput('');
      try { await notificationAPI.list(); } catch (e) {}
      try { window.dispatchEvent(new CustomEvent('notificationsChanged')); } catch (e) {}
      await load();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-3xl mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          <button aria-label="Close chat" onClick={onClose} className="p-2 rounded-md text-violet-700 hover:bg-violet-50">
        ...
        <div className="p-4 h-[60vh] flex flex-col">
          ...
          <div className="mt-3 flex gap-2">
            ...
            />
            <button onClick={send} disabled={loading || !input.trim()} className="w-12 h-12 rounded-lg bg-violet-600 text-white flex items-center justify-center hover:bg-violet-700 disabled:opacity-50">
